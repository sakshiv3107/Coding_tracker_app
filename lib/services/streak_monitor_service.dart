...
void callbackDispatcher() {
  Workmanager().executeTask((task, inputData) async {
    try {
      ...
      if (task == "streakMonitorTask" || task == "backgroundRefreshTask") {
        // 1. Refresh Contests & Schedule Notifications
        final contestsEnabled = prefs.getBool('contest_reminders_enabled') ?? true;
        if (contestsEnabled) {
          try {
            final contestService = ContestService();
            // This will fetch and schedule notifications
            await contestService.fetchUpcomingContests();
          } catch (e) {
            debugPrint('Background contest refresh error: $e');
          }
        }
      ...
      debugPrint('Background task general error: $e');
    }
